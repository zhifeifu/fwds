...
func NewDefaultAliOss() Driver {
	aliConfig := AliConfig{
		AccessKey: config.Conf.Oss.AliYun.AccessKey,
		SecretKey: config.Conf.Oss.AliYun.SecretKey,
		Bucket:    config.Conf.Oss.AliYun.Bucket,
		Endpoint:  config.Conf.Oss.AliYun.Endpoint,
	}
	return NewAliOss(aliConfig)
}
